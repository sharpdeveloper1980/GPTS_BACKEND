...
class SchoolController extends Controller {
    ...
    public function save(Request $request) {
       // echo $request->email;die();
        try {
            ...
            $data = array('name' => $request->name,'cperson_name'=>$request->contact_pre_name, 'title' => 'Registration with gpts');
            Mail::send('emails.freelancer.welcomemail', $data, function($message) use($recipient, $siteEmail, $recipientName) {
                $message->to($recipient, $recipientName)->subject('Welcome to GPTS');
                $message->bcc(['lakhan@greatplacetostudy.org','srishti@greatplacetostudy.org','megha@greatplacetostudy.org']);
                $message->from($siteEmail, 'Great Place To Study');
            });
    ...
    public function saveAssignCode(Request $request){
        ...
        Excel::create($exportFileName, function($excel) use ($assignCode) {
            ...
            $defaultData    = ['Generated Licenses Codes'];
            $allData[] = $defaultData;
        ...
        $data = array('school'=>$schoolinfo->name,'contactPerson' => $contactPerson,'title'=>'GPTS License Code','loginlink' => 'www.google.com');
        
        Mail::send('emails.liciencecode', $data, function($message) use($path, $recipient,$recipientName){
            $message->to($recipient, $recipientName)->subject('GPTS License Codes');
            $message->bcc(['lakhan@greatplacetostudy.org','srishti@greatplacetostudy.org','megha@greatplacetostudy.org']);
