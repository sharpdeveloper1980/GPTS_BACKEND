    ...
    public function saveCareer(Request $request) {
        try {
            ...
            if (isset($_FILES['cluster_image']['name']) && $_FILES['cluster_image']['name'] != ''):
                $careericon = $request->file('cluster_image');
                $imgname = time() . '.' . $careericon->getClientOriginalExtension();
                $s3 = \Storage::disk('s3');
                $filePath = '/career/discover/' . $imgname;
            ...
            if (isset($_FILES['career_image']['name']) && $_FILES['career_image']['name'] != ''):
                $career_image = $request->file('career_image');
                $imgname = time() . '.' . $career_image->getClientOriginalExtension();
                \Storage::disk('s3')->delete('/career/career-icon/' . $query->career_image);
                $s3 = \Storage::disk('s3');
                $filePath = '/career/career-icon/' . $imgname;
                $s3->put($filePath, file_get_contents($career_image), 'public');
                $career_image = $imgname; 
            else:
                $career_image = $query->career_image;
            endif;
            $query->career_image = $career_image;
            ...
            if (isset($_FILES['cluster_video']['name']) && $_FILES['cluster_video']['name'] != ''):
                $clustervideo = $request->file('cluster_video');
                $clustervideoname = time() . '.' . $clustervideo->getClientOriginalExtension();
                $s3 = \Storage::disk('s3');
                $filePath = '/career/discover/' . $clustervideoname;
                $s3->put($filePath, file_get_contents($clustervideo), 'public');
                $cluster_video = $clustervideoname;                                
            else:
                $cluster_video = '';
            endif;
            
            if (isset($_FILES['cluster_thumb']['name']) && $_FILES['cluster_thumb']['name'] != ''):
                $cluster_thumb = $request->file('cluster_thumb');
                $cluster_thumbname = time() . '.' . $cluster_thumb->getClientOriginalExtension();
                $s3 = \Storage::disk('s3');
                $filePath = '/career/discover/' . $cluster_thumbname;
                $s3->put($filePath, file_get_contents($cluster_thumb), 'public');
                $cluster_thumb = $cluster_thumbname;                                
            else:
                $cluster_thumb = '';
            endif;
            ...
            if (isset($_FILES['edieo_thumb']['name']) && $_FILES['edieo_thumb']['name'] != ''):
                $video_thumb= $request->file('edieo_thumb');
                $edieothumbname = 'edieo'.time() . '.' . $video_thumb->getClientOriginalExtension();
                $s3 = \Storage::disk('s3');
                $filePath = '/career/career-video/' . $edieothumbname;
                $s3->put($filePath, file_get_contents($video_thumb), 'public');
                $edieo_thumb_name = $edieothumbname;                                
            else:
                $edieo_thumb_name = '';
            endif;

            if (isset($_FILES['edieo_cluster_thumb']['name']) && $_FILES['edieo_cluster_thumb']['name'] != ''):
                $video_thumb= $request->file('edieo_cluster_thumb');
                $edieothumbname = 'edieo'.time() . '.' . $video_thumb->getClientOriginalExtension();
                $s3 = \Storage::disk('s3');
                $filePath = '/career/career-video/' . $edieothumbname;
                $s3->put($filePath, file_get_contents($video_thumb), 'public');
                $edieo_college_thumb_name = $edieothumbname;                                
            else:
                $edieo_college_thumb_name = '';
            endif;

            
             $query->exp_video = $exp_video;
             $query->cluster_video = $cluster_video;
             $query->cluster_thumb = $cluster_thumb;
             $query->video_thumb = $exp_thumb_name; 
             $query->edieo_thumb = $edieo_thumb_name;
             $query->edieo_cluster_thumb = $edieo_college_thumb_name;
            ...
            // Clusters
            $query->college_no=$request->college_no;
            $query->under_programme=$request->under_programme;
            $query->students_enrolled=$request->students_enrolled;
            $query->average_fees=$request->average_fees;
            $query->quotes=$request->quotes;
    ...
    public function edit(Request $request, $id) {
        try {
            ...
            if (isset($_FILES['edieo_thumb']['name']) && $_FILES['edieo_thumb']['name'] != ''):
                ...
                $edieo_thumb_name = $query->edieo_thumb;
            endif;
            ...
            if (isset($_FILES['edieo_cluster_thumb']['name']) && $_FILES['edieo_cluster_thumb']['name'] != ''):
                ...
                $edieo_college_thumb_name = $query->edieo_cluster_thumb;
            endif;
            
            $setClust = Career::whereId($id)->first();
            $setClust->edieo_thumb = $edieo_thumb_name;
            $setClust->edieo_cluster_thumb = $edieo_college_thumb_name;
            $setClust->save();

            if (isset($_FILES['cluster_image']['name']) && $_FILES['cluster_image']['name'] != ''):
                $careericon = $request->file('cluster_image');
                $imgname = time() . '.' . $careericon->getClientOriginalExtension();
                \Storage::disk('s3')->delete('/career/discover/' . $query->cluster_image);
                $s3 = \Storage::disk('s3');
                $filePath = '/career/discover/' . $imgname;
                $s3->put($filePath, file_get_contents($careericon), 'public');
                $cluster_image = $imgname; 
            ...
            if (isset($_FILES['cluster_video']['name']) && $_FILES['cluster_video']['name'] != ''):
                $clustervideo = $request->file('cluster_video');
                $clustervideoname = time() . '.' . $clustervideo->getClientOriginalExtension();
                \Storage::disk('s3')->delete('/career/discover/' . $query->cluster_video);
                $s3 = \Storage::disk('s3');
                $filePath = '/career/discover/' . $clustervideoname;
                $s3->put($filePath, file_get_contents($clustervideo), 'public');
                $cluster_video = $clustervideoname;                                
            else:
                $cluster_video = $query->cluster_video;
            endif;
...
//            $query->top_colleges = json_encode($request->top_college);
    ...
    public function destroy($type,$id) {
        try {
            ...
            \Storage::disk('s3')->delete('/career/career-icon/' . $career->career_image);
            ...
            if($type==1){
             ...
             \Storage::disk('s3')->delete('/career/discover/' . $career->cluster_image);
             \Storage::disk('s3')->delete('/career/discover/' . $career->cluster_video);
             \Storage::disk('s3')->delete('/career/discover/' . $career->cluster_thumb);
...
//                'about' => 'required',
        ...
        try {
           ...
           $validator = Validator::make($request->all(), [
            ...
            if ($validator->fails()) {
                ...
                if (isset($_FILES['edieo_thumb']['name']) && $_FILES['edieo_thumb']['name'] != ''):
                $beginner_thumb = $request->file('edieo_thumb');
    ...
    */
    ...
    public function changeVideoPriority($id){
        ...
        CareerLibaryVideo::where('id', '>', 0)->update(['priority_video' => 0]);
        CareerLibaryVideo::where('id', $id)->update(['priority_video' => 1]);
    }
}
