...
class EdieoController extends Controller {
    ...
    public function getEdieo(Request $request) {

        $cluster_id=$request->input('cluster_id');

        if($cluster_id!=''){
            $query=DB::select("SELECT slug FROM `gpts_career_library` where parent='".$cluster_id."'");
            ...
            $list = Edieo::whereIn('slug', $slugs)->whereStatus(1)->orderBy('position', 'ASC')->get();
        }else{
    ...
    public function getSingleVideo(Request $request){
       ...
       // print_r($request->all());die();
        if($request->type == 'cluster'){
            ...
            if(isset($edieo->career) && !empty($edieo->career) && $edieo->career>0){

            	$career=\DB::select("SELECT slug FROM gpts_career_library WHERE tte_career_id=".$edieo->career);
            	$data['pslug']      = $career[0]->slug;
