@include('emails.header2')

<table border="0" width="700" cellpadding="0" cellspacing="0" align="center" bgcolor="#ffffff" style="border: 1px solid #e5e5e5;margin: auto;" class="email-container">
    <!-- Full Width, Fluid Column : BEGIN -->
    <tr>
        <td style="border-bottom: 1px solid #e5e5e5;">
            <table border="0" width="100%" cellpadding="0" cellspacing="0" align="center">
                <tr style="padding: 30px;font-family: sans-serif;font-size: 16px;line-height: 0px;color: #444444;padding-left: 30px;display: block;margin-bottom: -15px;font-size: 20px;font-weight: 500;">
                    <td>Hello {{$name}},</td>
                </tr>
                <tr style="padding: 30px; font-family: sans-serif; font-size: 16px; line-height: 22px; color: #444444;padding-left: 30px;
                    display: block; font-size: 20px; margin-bottom: -35px;">
                    <td> {{$title}}. </td>
                </tr>
                <tr>
                    <td style="padding: 30px; font-family: sans-serif; font-size: 14px; line-height: 22px; color: #444444;padding-left: 30px;
                        display: block;"> Congratulations! {{$adduserrole}} has created your account with {{$userrole}} autority, please login with this email-id and password.
                        <br>

                        You may sign in to your Super Panel with the following credentials: 
                        <br>
                        <b> E-mail: </b> {{$email}} <br>
                        <b> Password : </b> {{$password}} <br>
                </tr>
                <tr style="
                    padding-left: 30px;
                    display: block;
                    padding-top: 7em;
                    padding-bottom: 7em;
                    ">
                    <td style="-webkit-border-radius: 3px;-moz-border-radius: 3px;border-radius: 3px;background-color: #2853b5;text-align: center;width: 200px;display: block;">
                        <a href="{{ url('/administrator') }}" style="color: #ffffff; font-family: sans-serif; font-size: 15px; line-height: 15px; text-align: center; text-decoration: none; display: block; padding: 15px 20px; border: 1px solid #333333; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px;"> <b>
                                <!--[if mso]>&nbsp;<![endif]-->Login to your Account
                                <!--[if mso]>&nbsp;<![endif]--></b> </a>
                    </td>
                </tr>
            </table>
        </td>
    </tr>
    <!-- Full Width, Fluid Column : END -->
</table>

@include('emails.footer2')